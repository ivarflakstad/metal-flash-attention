...
public class AttentionPerfTests<T: TensorFloatingPoint> {
  public init() {}
...
//      SIMD4(        160, 385,  4, granularity * 4),
  ...
  func testAttention(
    ...
    func runAttention(
      ...
      autoreleasepool {
        var tensors = Tensors<T>(config: config, backend: backend)
    ...
    func verifyResults(
      ...
      var mfaTensors = Tensors<T>(config: config, backend: backend)
      let mps = backend.equivalentMPS.tensorBackend
      var mpsTensors = Tensors<T>(mfaTensors, backend: mps)
...
extension AttentionPerfTests: MFATestCase {
...
struct Tensors<T: TensorFloatingPoint> {
  var q: Tensor<T>
  var k: Tensor<T>
  var v: Tensor<T>
  var o: Tensor<T>
