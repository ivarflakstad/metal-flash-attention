...
class CorrectnessTests<T: TensorFloatingPoint> {
  static var useBlockSparsity: Bool { return true }
  ...
  func testRandomGEMM(logProgress: Bool) {
    ...
    func testRandomSize(index: Int, ghost: Bool) {
      ...
      let DTypeRepr = (T.self == Float.self) ? "f32" : "f16"
      ...
      var mfa_D: Tensor<T>?
      if useBias {
        mps_D = Tensor<T>(shape: shapeD, randomUniform: 0..<1, backend: .mps)
...
//            expected: mps_C, expectedName: "MPS", parameters: params)
  ...
  func testRandomAttention(logProgress: Bool) {
    ...
    let allRandomInts: [SIMD4<Int>] = (0..<numTrials).map { i in
      ...
      let threshold = (T.self == Float.self) ? 128 : 256
...
extension CorrectnessTests: MFATestCase {
